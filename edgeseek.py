...
image = cv2.imread("D:/Python picture/MVIMG_20250812_170218.jpg")
img =image
class DocScanner(object):
    ...
    def get_corners(self, img):
        ...
        """
        lsd = cv2.createLineSegmentDetector(0)  # 创建检测器
        lines = lsd.detect(img)[0]  # 检测直线，返回 N×1×4 的数
    ...
    def get_contour(self, rescaled_image):
        ...
        if len(test_corners) >= 4:
            ...
            for quad in itertools.combinations(test_corners, 4):
                points = np.array(quad, dtype="float32")  # 保证类型
                points = DocScanner.order_points(points)
                points = points.reshape(4, 1, 2)  # 保证形状
                quads.append(points)
    ...
    def scan(self, image_path):
        ...
        warped = DocScanner.four_point_transform(orig, screenCnt * ratio)
       ...
       # save the transformed image
        OUTPUT_DIR = r"D:\Python picture"  # 注意使用原始字符串，避免转义符问题
        if not os.path.exists(OUTPUT_DIR):
            os.makedirs(OUTPUT_DIR)  # 如果文件夹不存在就创建
